...
fcyMemPool<sizeof(ResParticle::ParticlePool)> ResParticle::s_MemoryPool(512);
// 预分配512个对象
...
ResAnimation::ResAnimation(const char* name, fcyRefPointer<ResTexture> tex,
                           ...
                           bool rect)
  : Resource(ResourceType::Animation, name), m_Interval(intv), m_HalfSizeX(a),
    m_HalfSizeY(b), m_bRectangle(rect) {
  ...
  for (int j = 0; j < m; ++j) // 行
  {
    for (int i = 0; i < n; ++i) // 列
    {
      ...
      if (FCYFAILED(LAPP.GetRenderer()->CreateSprite2D(tex->GetTexture(),
        fcyRect(
          x + w * i, y + h * j, x + w * (i + 1), y + h * (j + 1)
        ), &t))) {
        throw fcyException("ResAnimation::ResAnimation",
                           "CreateSprite2D failed.");
      }
      t->SetZ(0.5f);
...
/// ResParticle
////////////////////////////////////////////////////////////////////////////////
ResParticle::ResParticle(const char* name, const ParticleInfo& pinfo,
                         fcyRefPointer<f2dSprite> sprite, BlendMode bld,
                         double a, double b, bool rect)
  : Resource(ResourceType::Particle, name), m_ParticleInfo(pinfo),
    m_BindedSprite(sprite), m_BlendMode(bld), m_HalfSizeX(a), m_HalfSizeY(b),
    m_bRectangle(rect) {}
...
void ResParticle::ParticlePool::Update(float delta) {
  ...
  if (m_iStatus == Status::Alive) {
    ...
    for (fuInt i = 0; i < nParticlesCreated; ++i) {
      ...
      tInst.fAge = 0.0f;
      tInst.fTerminalAge = s_ParticleRandomizer.GetRandFloat(
        pInfo.fParticleLifeMin, pInfo.fParticleLifeMax);

      tInst.vecLocation = m_vPrevCenter + (m_vCenter - m_vPrevCenter) *
                          s_ParticleRandomizer.GetRandFloat(0.0f, 1.0f);
      ...
      float ang = /* pInfo.fDirection */
        (m_fRotation - (float)LPI_HALF) - (float)LPI_HALF + s_ParticleRandomizer
        .GetRandFloat(0, pInfo.fSpread) - pInfo.fSpread / 2.0f;
      tInst.vecVelocity.x = cos(ang);
      tInst.vecVelocity.y = sin(ang);
      tInst.vecVelocity *= s_ParticleRandomizer.GetRandFloat(
        pInfo.fSpeedMin, pInfo.fSpeedMax);

      tInst.fGravity = s_ParticleRandomizer.GetRandFloat(
        pInfo.fGravityMin, pInfo.fGravityMax);
      tInst.fRadialAccel = s_ParticleRandomizer.GetRandFloat(
        pInfo.fRadialAccelMin, pInfo.fRadialAccelMax);
      tInst.fTangentialAccel = s_ParticleRandomizer.GetRandFloat(
        pInfo.fTangentialAccelMin, pInfo.fTangentialAccelMax);

      tInst.fSize = s_ParticleRandomizer.GetRandFloat(
        pInfo.fSizeStart,
        pInfo.fSizeStart + (pInfo.fSizeEnd - pInfo.fSizeStart) * pInfo.
        fSizeVar);
      tInst.fSizeDelta = (pInfo.fSizeEnd - tInst.fSize) / tInst.fTerminalAge;

      tInst.fSpin = /* pInfo.fSpinStart */
        m_fRotation + s_ParticleRandomizer.GetRandFloat(0, pInfo.fSpinEnd) -
        pInfo.fSpinEnd / 2.0f;
      tInst.fSpinDelta = pInfo.fSpinVar;

      tInst.colColor[0] = s_ParticleRandomizer.GetRandFloat(
        pInfo.colColorStart[0],
        pInfo.colColorStart[0] + (pInfo.colColorEnd[0] - pInfo.colColorStart[0])
        * pInfo.fColorVar);
      tInst.colColor[1] = s_ParticleRandomizer.GetRandFloat(
        pInfo.colColorStart[1],
        pInfo.colColorStart[1] + (pInfo.colColorEnd[1] - pInfo.colColorStart[1])
        * pInfo.fColorVar);
      tInst.colColor[2] = s_ParticleRandomizer.GetRandFloat(
        pInfo.colColorStart[2],
        pInfo.colColorStart[2] + (pInfo.colColorEnd[2] - pInfo.colColorStart[2])
        * pInfo.fColorVar);
      tInst.colColor[3] = s_ParticleRandomizer.GetRandFloat(
        pInfo.colColorStart[3],
        pInfo.colColorStart[3] + (pInfo.colColorEnd[3] - pInfo.colColorStart[3])
        * pInfo.fAlphaVar);

      tInst.colColorDelta[0] = (pInfo.colColorEnd[0] - tInst.colColor[0]) /
                               tInst.fTerminalAge;
      tInst.colColorDelta[1] = (pInfo.colColorEnd[1] - tInst.colColor[1]) /
                               tInst.fTerminalAge;
      tInst.colColorDelta[2] = (pInfo.colColorEnd[2] - tInst.colColor[2]) /
                               tInst.fTerminalAge;
      tInst.colColorDelta[3] = (pInfo.colColorEnd[3] - tInst.colColor[3]) /
                               tInst.fTerminalAge;
    }
  }
...
void ResParticle::ParticlePool::Render(f2dGraphics2D* graph, float scaleX,
                                       float scaleY) {
  ...
  for (size_t i = 0; i < m_iAlive; ++i) {
    ...
    if (pInfo.colColorStart[0] < 0) // r < 0
      ...
      p->SetColor(fcyColor(pInst.colColor[3], pInst.colColor[0],
                           pInst.colColor[1], pInst.colColor[2]));
    p->Draw2(graph, fcyVec2(pInst.vecLocation.x, pInst.vecLocation.y),
             fcyVec2(scaleX * pInst.fSize, scaleY * pInst.fSize), pInst.fSpin,
             false);
  }
...
/// ResFont
////////////////////////////////////////////////////////////////////////////////
void ResFont::HGEFont::ReadDefine(const std::wstring& data,
                                  std::unordered_map<wchar_t, f2dGlyphInfo>&
                                  out, std::wstring& tex) {
  out.clear();
  tex.clear();
  ...
  for (size_t i = 1; i < tLines.size(); ++i) {
    ...
    if (tKey == L"Bitmap")
      ...
      if (7 != swscanf(tValue.c_str(), L"\"%c\",%f,%f,%f,%f,%f,%f", &c, &x, &y,
                       &w, &h, &left_offset, &right_offset)) {
        if (7 != swscanf(tValue.c_str(), L"%X,%f,%f,%f,%f,%f,%f", &c_hex, &x,
                         &y, &w, &h, &left_offset, &right_offset))
          throw fcyException("ResFont::HGEFont::readDefine",
                             "Bad file format.");
      ...
      if (out.find(c) != out.end())
        throw fcyException("ResFont::HGEFont::readDefine",
                           "Duplicated character defination.");
      out.emplace(c, tInfo);
    } else
      throw fcyException("ResFont::HGEFont::readDefine", "Bad file format.");
  }
...
ResFont::HGEFont::HGEFont(std::unordered_map<wchar_t, f2dGlyphInfo>&& org,
...
fResult ResFont::HGEFont::QueryGlyph(f2dGraphics* pGraph, fCharW Character,
                                     f2dGlyphInfo* InfoOut) {
...
/// ResMusic
////////////////////////////////////////////////////////////////////////////////
fResult ResMusic::BGMWrapper::Read(fData pBuffer, fuInt SizeToRead,
                                   fuInt* pSizeRead) {
  fResult tFR;
  ...
  while (tSampleToRead) {
    ...
    if (tCurSample + tSampleToRead > m_pLoopEndSample) {
      ...
      // 直接填充数据
      if (FAILED(tFR = m_pDecoder->Read(pBuffer, tSampleToRead * tBlockAlign,
        pSizeRead)))
        return tFR;
...
ResMusic::BGMWrapper::BGMWrapper(fcyRefPointer<f2dSoundDecoder> pOrg,
                                 fDouble LoopStart, fDouble LoopEnd)
  : m_pDecoder(pOrg) {
  LASSERT(pOrg);
  ...
  if (LoopEnd <= 0)
    ...
    m_pLoopEndSample = min(m_TotalSample,
                           (fuInt)(LoopEnd * m_pDecoder->GetSamplesPerSec()));

  if (m_pLoopEndSample < m_pLoopStartSample)
    std::swap(m_pLoopStartSample, m_pLoopEndSample);

  if (m_pLoopEndSample == m_pLoopStartSample)
    throw fcyException("ResMusic::BGMWrapper::BGMWrapper",
                       "Invalid loop period.");
}
  ...
  for (fuInt i = 0; i < m_pShader->GetParamCount(); ++i) {
    ...
    if (pParam->GetType() == F2DEPTYPE_VALUE) {
      ...
      if (pAnnotation && pAnnotation->GetType() == F2DEPTYPE_VALUE) {
        ...
        if (pAnnotationVal->GetValueType() == F2DEPVTYPE_STRING &&
            pAnnotationVal->GetString()) {
      ...
      if (pSemantic) {
        if (_stricmp(pSemantic, "POSTEFFECTTEXTURE") == 0) {
          if (pValue->GetValueType() != F2DEPVTYPE_TEXTURE2D)
            throw fcyException("ResFX::ResFX",
                               "Invalid binding for 'POSTEFFECTTEXTURE'.");
...
void ResFX::SetValue(const char* key, fcyColor val)LNOEXCEPT {
  auto i = m_pBindingVar.find(key);
  if (i != m_pBindingVar.end()) {
    fcyVec4 tColorVec(val.r / 255.f, val.g / 255.f, val.b / 255.f,
                      val.a / 255.f);
...
void ResourcePool::ExportResourceList(lua_State* L,
                                      ResourceType t) const LNOEXCEPT {
  int cnt = 1;
  switch (t) {
    case ResourceType::Texture:
      lua_newtable(L); // t
...
bool ResourcePool::LoadTexture(const char* name, const std::wstring& path,
                               bool mipmaps)LNOEXCEPT {
...

  {
    ...
    if (FCYFAILED(LAPP.GetRenderDev()->CreateTextureFromMemory((fcData)tDataBuf
      ->GetInternalBuffer(), tDataBuf->GetLength(), 0, 0, false, mipmaps, &
      tTexture))) {
    ...
    LINFO("LoadTexture: 纹理'%s'已装载 -> '%m' (%s)", path.c_str(), name,
...
bool ResourcePool::LoadTexture(const char* name, const char* path,
                               bool mipmaps)LNOEXCEPT {
  try {
    return LoadTexture(
      name, fcyStringHelper::MultiByteToWideChar(path, CP_UTF8), mipmaps);
  } catch (const bad_alloc&) {
...
bool ResourcePool::LoadImage(const char* name, const char* texname,
...

  {
    ...
    if (FCYFAILED(LAPP.GetRenderer()->CreateSprite2D(pTex->GetTexture(), tRect,
      &pSprite))) {
      LERROR("LoadImage: 无法从纹理'%m'加载图像'%m' (CreateSprite2D failed)", texname,
        name);
      return false;
    }
...
bool ResourcePool::LoadAnimation(const char* name, const char* texname,
                                 double x, double y, double w, double h, int n,
                                 int m, int intv, double a, double b,
                                 bool rect) {
  LDEBUG_RESOURCETIMER;

  {
    ...
    try {
      ...
      tRes.DirectSet(new ResAnimation(name, pTex, (float)x, (float)y, (float)w,
                                      (float)h, n, m, intv, a, b, rect));
...
bool ResourcePool::LoadMusic(const char* name, const std::wstring& path,
                             double start, double end)LNOEXCEPT {
  LDEBUG_RESOURCETIMER;

  {
    ...
    try {
      ...
      if (FCYFAILED(LAPP.GetSoundSys()->CreateOGGVorbisDecoder(tDataBuf, &
        tDecoder))) {
      ...
      tWrapperedBuffer.
        DirectSet(new ResMusic::BGMWrapper(tDecoder, start, end));

      fcyRefPointer<f2dSoundBuffer> tBuffer;
      if (FCYFAILED(LAPP.GetSoundSys()->CreateDynamicBuffer(tWrapperedBuffer,
        LSOUNDGLOBALFOCUS, &tBuffer))) {
        LERROR(
          "LoadMusic: 无法创建音频缓冲区，文件'%s' (f2dSoundSys::CreateDynamicBuffer failed.)"
        , path.c_str());
        return false;
      }
      ...
      LERROR("LoadMusic: 解码文件'%s'的音频数据时发生错误，格式不支持？ (异常信息'%m' 源'%m')", path.c_str
        (), e.GetDesc(), e.GetSrc());
      return false;
    }
    catch (const bad_alloc&) {
...
LNOINLINE bool ResourcePool::LoadMusic(const char* name, const char* path,
                                       double start, double end)LNOEXCEPT {
  try {
    return LoadMusic(name, fcyStringHelper::MultiByteToWideChar(path, CP_UTF8),
                     start, end);
  } catch (const bad_alloc&) {
...
bool ResourcePool::LoadSound(const char* name,
                             const std::wstring& path)LNOEXCEPT {
  LDEBUG_RESOURCETIMER;

  {
    ...
    try {
      ...
      fcyRefPointer<f2dSoundBuffer> tBuffer;
      if (FCYFAILED(LAPP.GetSoundSys()->CreateStaticBuffer(tDecoder,
        LSOUNDGLOBALFOCUS, &tBuffer))) {
        LERROR(
          "LoadSound: 无法创建音频缓冲区，文件'%s' (f2dSoundSys::CreateStaticBuffer failed.)"
        , path.c_str());
        return false;
      }
      ...
      LERROR("LoadSound: 解码文件'%s'的音频数据时发生错误，格式不支持？ (异常信息'%m' 源'%m')", path.c_str
        (), e.GetDesc(), e.GetSrc());
      return false;
    }
    catch (const bad_alloc&) {
...
LNOINLINE bool ResourcePool::LoadSound(const char* name,
                                       const char* path)LNOEXCEPT {
  try {
...
bool ResourcePool::LoadParticle(const char* name, const std::wstring& path,
                                const char* img_name, double a, double b,
                                bool rect)LNOEXCEPT {
  LDEBUG_RESOURCETIMER;

  {
    ...
    if (!pSprite) {
      ...
      // 克隆一个精灵对象
      if (FCYFAILED(LAPP.GetRenderer()->CreateSprite2D(pSprite->GetSprite()->
        GetTexture(), pSprite->GetSprite()->GetTexRect(), pSprite->GetSprite()->
        GetHotSpot(), &pClone))) {
    ...
    try {
      ...
      memcpy(&tInfo, outBuf->GetInternalBuffer(),
             sizeof(ResParticle::ParticleInfo));
      ...
      tRes.DirectSet(
        new ResParticle(name, tInfo, pClone, tBlendInfo, a, b, rect));
...
LNOINLINE bool ResourcePool::LoadParticle(const char* name, const char* path,
                                          const char* img_name, double a,
                                          double b, bool rect)LNOEXCEPT {
  try {
    return LoadParticle(
      name, fcyStringHelper::MultiByteToWideChar(path, CP_UTF8), img_name, a, b,
      rect);
  } catch (const bad_alloc&) {
...
bool ResourcePool::LoadSpriteFont(const char* name, const std::wstring& path,
                                  bool mipmaps)LNOEXCEPT {
  LDEBUG_RESOURCETIMER;

  {
    ...
    try {
      if (tDataBuf->GetLength() > 0) {
        ...
        tFileData = fcyStringHelper::MultiByteToWideChar(
          string((const char*)tDataBuf->GetInternalBuffer(),
                 (size_t)tDataBuf->GetLength()), CP_UTF8);
      }
    } catch (const bad_alloc&) {
    ...
    // 读取HGE字体定义
    try {
      ResFont::HGEFont::ReadDefine(tFileData, tOutputCharset,
                                   tOutputTextureName);
    } catch (const fcyException& e) {
      LERROR("LoadFont: 装载HGE字体定义文件'%s'失败 (异常信息'%m' 源'%m')", path.c_str(), e.
        GetDesc(), e.GetSrc());
      return false;
    }
    catch (const bad_alloc&) {
    ...
    try {
      if (!m_pMgr->LoadFile(
        (fcyPathParser::GetPath(path) + tOutputTextureName).c_str(), tDataBuf))
        return false;
    } catch (const bad_alloc&) {
      LERROR("LoadFont: 内存不足");
      return false;
    ...
    try {
      ...
      tFontProvider.DirectSet(
        new ResFont::HGEFont(std::move(tOutputCharset), tTexture));
...
bool ResourcePool::LoadSpriteFont(const char* name, const std::wstring& path,
...

  {
    ...
    // 装载纹理
    try {
      if (!m_pMgr->LoadFile((fcyPathParser::GetPath(path) + tex_path).c_str(),
                            tDataBuf)) {
    ...
    try {
      fcyRefPointer<f2dFontProvider> tFontProvider;
      if (FCYFAILED(LAPP.GetRenderer()->CreateFontFromTex(tFileData.c_str(),
        tTexture, &tFontProvider))) {
...
LNOINLINE bool ResourcePool::LoadSpriteFont(const char* name, const char* path,
                                            bool mipmaps)LNOEXCEPT {
  try {
...
LNOINLINE bool ResourcePool::LoadSpriteFont(const char* name, const char* path,
  ...
  try {
    return LoadSpriteFont(
      name, fcyStringHelper::MultiByteToWideChar(path, CP_UTF8),
...
bool ResourcePool::LoadTTFFont(const char* name, const std::wstring& path,
                               float width, float height)LNOEXCEPT {
  LDEBUG_RESOURCETIMER;

  {
    ...
    if (!m_pMgr->LoadFile(path.c_str(), tDataBuf)) {
      ...
      if (FCYFAILED(LAPP.GetRenderer()->CreateSystemFont(path.c_str(), 0,
        fcyVec2(width, height), F2DFONTFLAG_NONE, &tFontProvider))) {
    ...
    try {
      if (!tFontProvider) {
        if (FCYFAILED(LAPP.GetRenderer()->CreateFontFromFile(tDataBuf, 0,
          fcyVec2(width, height), F2DFONTFLAG_NONE, &tFontProvider))) {
...
LNOINLINE bool ResourcePool::LoadTTFFont(const char* name, const char* path,
                                         float width, float height)LNOEXCEPT {
  try {
    return LoadTTFFont(
      name, fcyStringHelper::MultiByteToWideChar(path, CP_UTF8), width, height);
  } catch (const bad_alloc&) {
...

  {
    ...
    try {
      ...
      if (FCYFAILED(LAPP.GetRenderDev()->CreateEffect(tDataBuf, false, &tEffect)
        ...
        LERROR("LoadFX: 加载shader于文件'%s'失败 (lasterr=%m)", path.c_str(), LAPP.
          GetEngine()->GetLastErrDesc());
        return false;
      }
      ...
      LERROR("LoadFX: 绑定变量于文件'%s'失败 (异常信息'%m' 源'%m')", path.c_str(), e.GetDesc()
    , e.GetSrc());
      return false;
    }
    catch (const bad_alloc&) {
...
LNOINLINE bool ResourcePool::
LoadFX(const char* name, const char* path)LNOEXCEPT {
  try {
...
LNOINLINE bool ResourcePool::CreateRenderTarget(const char* name)LNOEXCEPT {
...

  {
    ...
    fcyRefPointer<f2dTexture2D> tTexture;
    if (FCYFAILED(LAPP.GetRenderDev()->CreateRenderTarget(LAPP.GetRenderDev()->
      GetBufferWidth(),
...
ResourcePack::ResourcePack(const wchar_t* path, const char* passwd)
  ...
  if (!m_zipFile) {
    ...
    throw fcyException("ResourcePack::ResourcePack",
                       "Can't open resource pack.");
  }
}
...
bool ResourcePack::LoadFile(const wchar_t* path,
                            fcyRefPointer<fcyMemStream>& outBuf)LNOEXCEPT {
  string tPathInUtf8;
  try {
  ...
  while (UNZ_OK == tStatus) {
    ...
    if (UNZ_OK == unzGetCurrentFileInfo(m_zipFile, &tFileInfo, tZipName,
                                        sizeof(tZipName), nullptr, 0, nullptr,
                                        0)) {
      // 对路径做统一性转换
      ...
      if (strcmp(tPathInUtf8.c_str(), tZipName) == 0) {
...
#endif
        if (unzOpenCurrentFilePassword(m_zipFile,
                                       m_Password.length() > 0
                                       ? m_Password.c_str()
                                       : nullptr) != UNZ_OK) {
          LERROR("ResourcePack: 尝试打开资源包'%s'中的文件'%s'失败(密码错误?)", m_Path.c_str(),
            path);
          return false;
        }
        ...
        try {
          outBuf.DirectSet(
            new fcyMemStream(NULL, tFileInfo.uncompressed_size, true, false));
        } catch (const bad_alloc&) {
        ...
        if (outBuf->GetLength() > 0) {
          if (unzReadCurrentFile(m_zipFile, outBuf->GetInternalBuffer(),
                                 tFileInfo.uncompressed_size) < 0) {
            unzCloseCurrentFile(m_zipFile);
            LERROR("ResourcePack: 解压资源包'%s'中的文件'%s'失败 (unzReadCurrentFile失败)",
              m_Path.c_str(), path);
            return false;
          }
        }
      ...
      LWARNING("ResourcePack: 在资源包'%s'中寻找文件时发生错误 (unzGetCurrentFileInfo失败)",
      m_Path.c_str());
...
LNOINLINE bool ResourceMgr::LoadPack(const char* path,
                                     const char* passwd)LNOEXCEPT {
  try {
...
LNOINLINE bool ResourceMgr::LoadFile(const wchar_t* path,
                                     fcyRefPointer<fcyMemStream>& outBuf)
LNOEXCEPT {
  // 尝试从各个资源包加载
  for (auto& i : m_ResPackList) {
  ...
  try {
    ...
    LERROR("ResourceMgr: 装载本地文件'%s'失败，文件不存在？ (异常信息'%m' 源'%m')", path, e.GetDesc(
    ), e.GetSrc());
    return false;
  }

  if (pFile->GetLength() > 0) {
    if (FCYFAILED(pFile->ReadBytes((fData)outBuf->GetInternalBuffer(), outBuf->
      GetLength(), nullptr))) {
...
LNOINLINE bool ResourceMgr::LoadFile(const char* path,
                                     fcyRefPointer<fcyMemStream>& outBuf)
LNOEXCEPT {
  try {
    wstring tPath = fcyStringHelper::MultiByteToWideChar(path, CP_UTF8);
...
bool ResourceMgr::ExtractRes(const wchar_t* path,
                             const wchar_t* target)LNOEXCEPT {
  ...
  if (LoadFile(path, tBuf)) {
    ...
    try {
      ...
      if (tBuf->GetLength() > 0) {
        if (FCYFAILED(pFile->WriteBytes((fcData)tBuf->GetInternalBuffer(), tBuf
          ->GetLength(), nullptr))) {
      ...
      LERROR("ResourceMgr: 打开本地文件'%s'失败 (异常信息'%m' 源'%m')", target, e.GetDesc(),
        e.GetSrc());
      return false;
    }
  }
...
LNOINLINE bool ResourceMgr::ExtractRes(const char* path,
                                       const char* target)LNOEXCEPT {
  try {
    wstring tPath = fcyStringHelper::MultiByteToWideChar(path, CP_UTF8);
