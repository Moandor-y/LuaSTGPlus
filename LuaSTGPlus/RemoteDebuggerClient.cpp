...
RemoteDebuggerClient::RemoteDebuggerClient(fuShort p, const char* addr)
  ...
  if (tRet != 0)
    throw fcyException("RemoteDebuggerClient::RemoteDebuggerClient",
                       "WSAStartup failed.");
  ...
  m_SendPacketCache->VDict["processId"] = make_shared<Value>(
...
void RemoteDebuggerClient::sendUdpMessage(UdpMessageType type,
                                          std::shared_ptr<Value> val) {
  m_SendPacketCache->VDict["msgType"] = make_shared<Value>(
  ...
  // 并不理会sendto成功与否
  sendto(S, tData.c_str(), tData.size(), 0, (sockaddr*)&tEndpoint,
         tEndpointLen);
}

void RemoteDebuggerClient::SendPerformanceCounter(
  float FPS, float ObjCount, float FrameTime, float RenderTime) {
  ...
  tMessage->VDict["objects"] = make_shared<Value>(
    static_cast<int>(ObjCount * 1000.f));
  tMessage->VDict["frametime"] = make_shared<Value>(
    static_cast<int>(FrameTime * 1000.f * 1000.f));
  tMessage->VDict["rendertime"] = make_shared<Value>(
    static_cast<int>(RenderTime * 1000.f * 1000.f));

  sendUdpMessage(UdpMessageType::PerformanceUpdate, tMessage);
}

void RemoteDebuggerClient::SendResourceLoadedHint(
  ResourceType Type, ResourcePoolType PoolType, const char* Name,
  const wchar_t* Path, float LoadingTime) {
  shared_ptr<Value> tMessage = make_shared<Value>(ValueType::Dictionary);
  ...
  tMessage->VDict["path"] = make_shared<Value>(
    fcyStringHelper::WideCharToMultiByte(Path, CP_UTF8));
  tMessage->VDict["time"] = make_shared<Value>(
    static_cast<int>(LoadingTime * 1000.f));

  sendUdpMessage(UdpMessageType::ResourceLoaded, tMessage);
}

void RemoteDebuggerClient::SendResourceRemovedHint(
  ResourceType Type, ResourcePoolType PoolType, const char* Name) {
  shared_ptr<Value> tMessage = make_shared<Value>(ValueType::Dictionary);
  tMessage->VDict["type"] = make_shared<Value>(static_cast<int>(Type));
  tMessage->VDict["pool"] = make_shared<Value>(static_cast<int>(PoolType));
