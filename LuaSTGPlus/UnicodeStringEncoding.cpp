...
public:
  bool operator()(uint8_t input, char32_t& output) {
    ...
    if (m_iState == 0) {
      // 前导字节
      if (input >= 0xFCu && input <= 0xFDu)
        // UCS4 1111110x 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx
      {
        m_incpChar = input & 0x01u;
        m_iState = 5;
      } else if (input >= 0xF8u && input <= 0xFBu)
        // UCS4 111110xx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx
      {
        m_incpChar = input & 0x03u;
        m_iState = 4;
      } else if (input >= 0xF0u && input <= 0xF7u)
        // UCS4 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
      {
        ...
        // 0800~FFFF: 1110xxxx 10xxxxxx 10xxxxxx
      {
        ...
        m_iState = 2;
      } else if (input >= 0xC0u && input <= 0xDFu)
          ...
          *((uint8_t*)&tempChar + 1) = m_cBuf[1];
          ...
          if (tempChar >= 0xDC00u && tempChar < 0xE000) {
            output = static_cast<char32_t>(
              (m_leadChar << 16) | (tempChar - 0xDC00u));
            m_iState = 0;
            return true;
          } else {
            output = (char32_t)'?';
...
template<typename decoder_t, typename encoder_t, typename char_t = char,
         typename orgchar_t>
void Convert(const orgchar_t* nullTerminatedString,
