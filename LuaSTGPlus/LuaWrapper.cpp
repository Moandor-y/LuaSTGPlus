...
static inline void TranslateAlignMode(lua_State* L, int argnum,
...
void ColorWrapper::Register(lua_State* L)LNOEXCEPT {
  struct WrapperImplement {
    ...
    static int Meta_Eq(lua_State* L)LNOEXCEPT {
      fcyColor* pA = static_cast<fcyColor*>(luaL_checkudata(
        L, 1, TYPENAME_COLOR));
      fcyColor* pB = static_cast<fcyColor*>(luaL_checkudata(
        L, 2, TYPENAME_COLOR));
...
void RandomizerWrapper::Register(lua_State* L)LNOEXCEPT {
  struct WrapperImplement {
    static int Seed(lua_State* L)LNOEXCEPT {
      fcyRandomWELL512* p = static_cast<fcyRandomWELL512*>(luaL_checkudata(
        L, 1, TYPENAME_RANDGEN));
    ...
    static int Int(lua_State* L)LNOEXCEPT {
      ...
      lua_pushinteger(
        L, a + static_cast<fInt>(p->GetRandUInt(
             ::max(static_cast<fuInt>(b - a), 0U))));
      return 1;
    }
...
fcyRandomWELL512* RandomizerWrapper::CreateAndPush(lua_State* L) {
  fcyRandomWELL512* p = static_cast<fcyRandomWELL512*>(lua_newuserdata(
    L, sizeof(fcyRandomWELL512)));
...
void BentLaserDataWrapper::Register(lua_State* L)LNOEXCEPT {
  struct WrapperImplement {
    static int Update(lua_State* L)LNOEXCEPT {
      Wrapper* p = static_cast<Wrapper*>(luaL_checkudata(
        L, 1, TYPENAME_BENTLASER));
      if (!p->handle)
      ...
      lua_pop(L, 1);
      if (!p->handle->Update(id, luaL_checkinteger(L, 3),
    ...
    static int Render(lua_State* L)LNOEXCEPT {
      ...
      if (!p->handle->Render(
        ...
        *static_cast<fcyColor*>(luaL_checkudata(L, 4, TYPENAME_COLOR)),
        ...
        return luaL_error(L, "can't render object with texture '%s'.",
                          luaL_checkstring(L, 2));
      }
      return 0;
    }
  ...
  struct WrapperImplement {
    ...
    static int ExtractRes(lua_State* L)LNOEXCEPT {
      ...
      if (!LRES.ExtractRes(pArgPath, pArgTarget))
        return luaL_error(L, "failed to extract resource '%s' to '%s'.",
                          pArgPath, pArgTarget);
      return 0;
    }
    ...
    static int Angle(lua_State* L)LNOEXCEPT {
      if (lua_gettop(L) == 2) {
        ...
        double tRet;
        if (!LPOOL.Angle((size_t)luaL_checkint(L, -2),
                         (size_t)luaL_checkint(L, -1), tRet))
          return luaL_error(L, "invalid lstg object for 'Angle'.");
        lua_pushnumber(L, tRet);
      } else {
        lua_pushnumber(L,
                       atan2(luaL_checknumber(L, 4) - luaL_checknumber(L, 2),
                             luaL_checknumber(L, 3) - luaL_checknumber(L, 1)) *
                       LRAD2DEGREE
        );
      }
      return 1;
    ...
    static int Dist(lua_State* L)LNOEXCEPT {
      if (lua_gettop(L) == 2) {
        ...
        lua_rawgeti(L, 2, 2); // t(object) t(object) id id
        double tRet;
        if (!LPOOL.Dist((size_t)luaL_checkint(L, -2),
                        (size_t)luaL_checkint(L, -1), tRet))
          return luaL_error(L, "invalid lstg object for 'Dist'.");
        lua_pushnumber(L, tRet);
      } else {
    ...
    static int SetResourceStatus(lua_State* L)LNOEXCEPT {
      ...
      if (strcmp(s, "global") == 0)
        ...
        return luaL_error(
          L,
          "invalid argument #1 for 'SetResourceStatus', requires 'stage', 'global' or 'none'.");
      return 0;
    }
    ...
    static int LoadImage(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->LoadImage(
        ...
        return luaL_error(L, "load image failed (name='%s', tex='%s').", name,
                          texname);
      }
      return 0;
    }
    ...
    static int LoadAnimation(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->LoadAnimation(
        ...
        return luaL_error(L, "load animation failed (name='%s', tex='%s').",
                          name, texname);
      }
    ...
    static int LoadPS(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->LoadParticle(
        ...
        return luaL_error(
          L, "load particle failed (name='%s', file='%s', img='%s').", name,
          path, img_name);
      }
    ...
    static int LoadSound(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->LoadSound(name, path))
        return luaL_error(L, "load sound failed (name=%s, path=%s)", name,
                          path);
      return 0;
    }
    ...
    static int LoadMusic(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->LoadMusic(
        ...
        return luaL_error(L, "load music failed (name=%s, path=%s, loop=%f~%f)",
                          name, path, loop_start, loop_end);
      }
      return 0;
    }
    ...
    static int LoadFont(lua_State* L)LNOEXCEPT {
      ...
      if (lua_gettop(L) == 2) {
        ...
        if (lua_isboolean(L, 3)) {
          ...
          bSucceed = pActivedPool->LoadSpriteFont(
            name, path, lua_toboolean(L, 3) == 0 ? false : true);
        } else {
          // fancy2d字体
          const char* texpath = luaL_checkstring(L, 3);
          if (lua_gettop(L) == 4)
            bSucceed = pActivedPool->LoadSpriteFont(
              name, path, texpath, lua_toboolean(L, 4) == 0 ? false : true);
          else
    ...
    static int LoadTTF(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->LoadTTFFont(name, path, (float)luaL_checknumber(L, 3),
                                     (float)luaL_checknumber(L, 4)))
        return luaL_error(L, "load ttf font failed (name=%s, path=%s)", name,
                          path);
      return 0;
    }
    ...
    static int CreateRenderTarget(lua_State* L)LNOEXCEPT {
      ...
      if (!pActivedPool->CreateRenderTarget(name))
        return luaL_error(L, "can't create render target with name '%s'.",
                          name);
      return 0;
    }
    ...
    static int IsRenderTarget(lua_State* L)LNOEXCEPT {
      ...
      if (!p)
        return luaL_error(L, "render target '%s' not found.",
    ...
    static int RemoveResource(lua_State* L)LNOEXCEPT {
      ...
      if (strcmp(s, "global") == 0)
        ...
        return luaL_error(
          L,
          "invalid argument #1 for 'RemoveResource', requires 'stage', 'global' or 'none'.");
      ...
      if (lua_gettop(L) == 1) {
        ...
        ResourceType tResourceType = static_cast<ResourceType>(luaL_checkint(L,
          2));
        const char* tResourceName = luaL_checkstring(L, 3);

        switch (t) {
          case ResourcePoolType::Stage:
            LRES.GetResourcePool(ResourcePoolType::Stage)->RemoveResource(
              tResourceType, tResourceName);
            break;
          case ResourcePoolType::Global:
            LRES.GetResourcePool(ResourcePoolType::Global)->RemoveResource(
              tResourceType, tResourceName);
            break;
          default:
            break;
    ...
    static int CheckRes(lua_State* L)LNOEXCEPT {
      ...
      // 先在全局池中寻找再到关卡池中找
      if (LRES.GetResourcePool(ResourcePoolType::Global)->CheckResourceExists(
        tResourceType, tResourceName))
        lua_pushstring(L, "global");
      else if (LRES.GetResourcePool(ResourcePoolType::Stage)->
        CheckResourceExists(tResourceType, tResourceName))
    ...
    static int EnumRes(lua_State* L)LNOEXCEPT {
      ...
      LRES.GetResourcePool(ResourcePoolType::Global)->ExportResourceList(
        L, tResourceType);
      LRES.GetResourcePool(ResourcePoolType::Stage)->ExportResourceList(
        L, tResourceType);
      return 2;
    }
    ...
    static int SetFontState(lua_State* L)LNOEXCEPT {
      ...
      if (!p)
        return luaL_error(L, "sprite font '%s' not found.",
                          luaL_checkstring(L, 1));

      p->SetBlendMode(TranslateBlendMode(L, 2));
      if (lua_gettop(L) == 3) {
        fcyColor c = *static_cast<fcyColor*>(luaL_checkudata(
          L, 3, TYPENAME_COLOR));
        p->SetBlendColor(c);
      }
      return 0;
    ...
    static int SetAnimationState(lua_State* L)LNOEXCEPT {
      ...
      if (!p)
        return luaL_error(L, "animation '%s' not found.",
                          luaL_checkstring(L, 1));
    ...
    static int SetImageCenter(lua_State* L)LNOEXCEPT {
      ...
      p->GetSprite()->SetHotSpot(fcyVec2(
        static_cast<float>(luaL_checknumber(L, 2) + p->GetSprite()->GetTexRect()
                           .a.x),
        static_cast<float>(luaL_checknumber(L, 3) + p->GetSprite()->GetTexRect()
                           .a.y)));
      return 0;
    }
    ...
    static int SetAnimationCenter(lua_State* L)LNOEXCEPT {
      ...
      for (size_t i = 0; i < p->GetCount(); ++i) {
        p->GetSprite(i)->SetHotSpot(fcyVec2(
          static_cast<float>(luaL_checknumber(L, 2) + p->GetSprite(i)->
                             GetTexRect().a.x),
          static_cast<float>(luaL_checknumber(L, 3) + p->GetSprite(i)->
                             GetTexRect().a.y)));
      }
      return 0;
    }
    ...
    static int Render(lua_State* L)LNOEXCEPT {
      if (!LAPP.Render(
        ...
        static_cast<float>(luaL_optnumber(L, 5, 1.) * LRES.
                           GetGlobalImageScaleFactor()),
        static_cast<float>(luaL_optnumber(L, 6, luaL_optnumber(L, 5, 1.)) * LRES
                           .GetGlobalImageScaleFactor()),
    ...
    static int RenderText(lua_State* L)LNOEXCEPT {
      ResFont::FontAlignHorizontal halign = ResFont::FontAlignHorizontal::
        Center;
    ...
    static int RenderTexture(lua_State* L)LNOEXCEPT {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        lua_pushinteger(L, 6);
        lua_gettable(L, 3 + i);
        vertex[i].color = static_cast<fcyColor*>(luaL_checkudata(
          L, -1, TYPENAME_COLOR))->argb;
    ...
    static int PushRenderTarget(lua_State* L)LNOEXCEPT {
      ResTexture* p = LRES.FindTexture(luaL_checkstring(L, 1));
      if (!p)
        return luaL_error(L, "rendertarget '%s' not found.",
                          luaL_checkstring(L, 1));
      if (!p->IsRenderTarget())
        return luaL_error(L, "'%s' is a texture.", luaL_checkstring(L, 1));

      if (!LAPP.PushRenderTarget(p))
        return luaL_error(L, "push rendertarget '%s' failed.",
                          luaL_checkstring(L, 1));
      return 0;
    }
      ...
      if (lua_istable(L, 4)) {
        ...
        while (0 != lua_next(L, 4)) {
          ...
          if (lua_isnumber(L, -1))
            ...
            if (!pTex)
              return luaL_error(L, "PostEffect: can't find texture '%s'.",
                                lua_tostring(L, -1));
      ...
      if (lua_istable(L, 3)) {
        ...
        while (0 != lua_next(L, 3)) {
          ...
          if (lua_isnumber(L, -1))
            ...
            ResTexture* pTex = LRES.FindTexture(lua_tostring(L, -1));
            if (!pTex)
              return luaL_error(L, "PostEffectApply: can't find texture '%s'.",
                                lua_tostring(L, -1));
            p->SetValue(key, pTex->GetTexture());
          } else if (lua_isuserdata(L, -1)) {
            fcyColor c = *static_cast<fcyColor*>(luaL_checkudata(
    ...
    static int PlaySound(lua_State* L)LNOEXCEPT {
      ...
      p->Play((float)luaL_checknumber(L, 2) * LRES.GetGlobalSoundEffectVolume(),
    ...
    static int PlayMusic(lua_State* L)LNOEXCEPT {
      ...
      p->Play((float)luaL_optnumber(L, 2, 1.) * LRES.GetGlobalMusicVolume(),
    ...
    static int Execute(lua_State* L)LNOEXCEPT {
      struct Detail_ {
        LNOINLINE static bool Execute(const char* path, const char* args,
                                      const char* directory, bool bWait,
                                      bool bShow)LNOEXCEPT {
          ...
          try {
            ...
            if (directory)
              tDirectory = fcyStringHelper::MultiByteToWideChar(
                directory, CP_UTF8);
    ...
    static int ATan2(lua_State* L)LNOEXCEPT {
      lua_pushnumber(
        L, atan2(luaL_checknumber(L, 1), luaL_checknumber(L, 2)) * LRAD2DEGREE);
      return 1;
    }
