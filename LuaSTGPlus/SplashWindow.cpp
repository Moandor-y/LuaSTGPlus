...
Gdiplus::Image* SplashWindow::LoadImageFromResource(UINT nID, LPCTSTR sTR) {
  HRSRC hRsrc = ::
    FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(nID), sTR);
  if (!hRsrc)
    return NULL;
...
LRESULT WINAPI SplashWindow::WndProc(HWND hWnd, UINT wMsg, WPARAM wParam,
                                     LPARAM lParam) {
  SplashWindow* pThis = (SplashWindow*)(LONG_PTR)GetWindowLongPtr(
    hWnd, GWLP_USERDATA);
...
void SplashWindow::createWindow() {
  ...
  HBITMAP bmpDest;
  bmpDest = CreateCompatibleBitmap(dcDest, m_bkImage->GetWidth(),
                                   m_bkImage->GetHeight());
  ...
  UpdateLayeredWindow(m_hHandle, dcDest, &tZero, &tSize, dcSrc, &tZero, 0,
                      &tBlend, ULW_ALPHA);
  ...
  // 设置窗体位置
  SetWindowPos(m_hHandle, 0, rect.left, rect.top, rect.right, rect.bottom,
               SWP_SHOWWINDOW);
