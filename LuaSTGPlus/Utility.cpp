...
std::string LuaSTGPlus::StringFormatV(const char* Format,
                                      va_list vaptr)LNOEXCEPT {
  std::string tRet;
  try {
    while (*Format != '\0') {
      char c = *Format;
...
std::wstring LuaSTGPlus::StringFormatV(const wchar_t* Format,
                                       va_list vaptr)LNOEXCEPT {
  std::wstring tRet;
  try {
    while (*Format != L'\0') {
      ...
      if (c != L'%')
        ...
        switch (c) {
          ...
          case 'm': {
            ...
            if (p)
              tRet.append(
                std::move(fcyStringHelper::MultiByteToWideChar(p, CP_UTF8)));
            else
              tRet.append(L"<null>");
          }
            break;
