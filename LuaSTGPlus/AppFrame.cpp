...
protected:
  fBool OnGlyphBeginDraw(fuInt Index, fCharW Character, fcyVec2& DrawPos,
                         fcyVec2& Adv) {
...
public:
  FontBaseLineDebugHelper(f2dGraphics2D* G, f2dGeometryRenderer* GR,
                          fcyRect BaseRect)
...
AppFrame::~AppFrame() {
  if (m_iStatus != AppStatus::NotInitialized && m_iStatus != AppStatus::
      Destroyed) {
    // 若没有销毁框架，则执行销毁
    Shutdown();
  }
    ...
    try {
      ...
      if (imgPath) {
        ...
        if (m_ResourceMgr.LoadFile(imgPath, tDataBuf))
          pImg = SplashWindow::LoadImageFromMemory(
            (fcData)tDataBuf->GetInternalBuffer(),
            (size_t)tDataBuf->GetLength());
...
LNOINLINE bool AppFrame::
ChangeVideoMode(int width, int height, bool windowed, bool vsync)LNOEXCEPT {
  // 切换到新的视频选项
  if (FCYOK(m_pRenderDev->SetBufferSize(
    ...
    F2DAALEVEL_NONE))) {
    LINFO(
      "视频模式切换成功 (%dx%d Vsync:%b Windowed:%b) -> (%dx%d Vsync:%b Windowed:%b)",
      (int)m_OptionResolution.x, (int)m_OptionResolution.y, m_OptionVsync,
      m_OptionWindowed,
    ...
    // 切换窗口大小
    m_pMainWindow->SetBorderType(m_OptionWindowed
      ? F2DWINBORDERTYPE_FIXED
      : F2DWINBORDERTYPE_NONE);
    m_pMainWindow->SetClientRect(
      fcyRect(10.f, 10.f, 10.f + m_OptionResolution.x,
              10.f + m_OptionResolution.y)
    );
    ...
    LINFO(
      "视频模式切换失败 (%dx%d Vsync:%b Windowed:%b) -> (%dx%d Vsync:%b Windowed:%b)",
      (int)m_OptionResolution.x, (int)m_OptionResolution.y, m_OptionVsync,
      m_OptionWindowed,
      width, height, vsync, windowed);
  }
...
LNOINLINE void AppFrame::LoadScript(const char* path)LNOEXCEPT {
  ...
  if (luaL_loadbuffer(L, (fcStr)tMemStream->GetInternalBuffer(),
                      (size_t)tMemStream->GetLength(),
...
fBool AppFrame::GetKeyState(int VKCode)LNOEXCEPT {
  if (VKCode > 0 && VKCode < _countof(m_KeyStateMap)) {
    if (LJOYSTICK1_MAPPING_START <= VKCode && VKCode <= LJOYSTICK1_MAPPING_END)
      // joystick1映射区域
    {
      if (m_Joystick[0]) {
        switch (VKCode) {
          ...
          default:
            return m_Joystick[0]->IsButtonDown(
              VKCode - LJOYSTICK1_MAPPING_START - 4);
        }
      } else
        return false;
    } else if (
      LJOYSTICK2_MAPPING_START <= VKCode && VKCode <= LJOYSTICK2_MAPPING_END)
      // joystick2映射区域
    {
      if (m_Joystick[1]) {
        switch (VKCode) {
          ...
          default:
            return m_Joystick[1]->IsButtonDown(
              VKCode - LJOYSTICK2_MAPPING_START - 4);
        }
      } else
        return false;
    } else if (m_Keyboard)
...
LNOINLINE int AppFrame::GetLastChar(lua_State* L)LNOEXCEPT {
  if (m_LastChar) {
    try {
      ...
      lua_pushstring(L,
                     fcyStringHelper::WideCharToMultiByte(tBuf, CP_UTF8).
                     c_str());
    } catch (const bad_alloc&) {
...
bool AppFrame::RenderText(ResFont* p, wchar_t* strBuf, fcyRect rect,
                          fcyVec2 scale,
                          ResFont::FontAlignHorizontal halign,
                          ResFont::FontAlignVertical valign,
                          bool bWordBreak)LNOEXCEPT {
  if (m_GraphType != GraphicsType::Graph2D) {
  ...
  // 计算起笔位置
  float fTotalLineHeight = pFontProvider->GetLineHeight() * iLineCount * scale.
                           y;
  fcyVec2 vRenderPos;
  switch (valign) {
  ...
  while (!bEOS) {
    ...
    switch (halign) {
      ...
      case ResFont::FontAlignHorizontal::Center:
        m_FontRenderer->DrawTextW2(m_Graph2D, pText,
                                   fcyVec2(
                                     vRenderPos.x + rect.GetWidth() / 2.f -
                                     fLineWidth / 2.f,
                                     vRenderPos.y));
        break;
...
fcyVec2 AppFrame::CalcuTextSize(ResFont* p, const wchar_t* strBuf,
                                fcyVec2 scale)LNOEXCEPT {
  if (m_GraphType != GraphicsType::Graph2D) {
    LERROR("RenderText: 只有2D渲染器可以执行该方法");
    return false;
  }
  ...
  return fcyVec2(fMaxLineWidth,
                 iLineCount * pFontProvider->GetLineHeight() * scale.y);
}

LNOINLINE bool AppFrame::RenderText(const char* name, const char* str, float x,
                                    float y,
                                    float scale,
                                    ResFont::FontAlignHorizontal halign,
                                    ResFont::FontAlignVertical valign)
LNOEXCEPT {
  ...
  // 计算渲染位置
  fcyVec2 tSize = CalcuTextSize(p, s_TempStringBuf.c_str(),
                                fcyVec2(scale, scale));
  switch (halign) {
    case ResFont::FontAlignHorizontal::Right:
      x -= tSize.x;
...
LNOINLINE bool AppFrame::RenderTTF(const char* name, const char* str,
                                   ...
                                   float scale, int format,
                                   fcyColor c)LNOEXCEPT {
...
LNOINLINE void AppFrame::SnapShot(const char* path)LNOEXCEPT {
  ...
  try {
    ...
    tOutputFile.DirectSet(
      new fcyFileStream(
        fcyStringHelper::MultiByteToWideChar(path, CP_UTF8).c_str(), true));
...
bool AppFrame::PostEffect(fcyRefPointer<f2dTexture2D> rt, ResFX* shader,
                          BlendMode blend)LNOEXCEPT {
  ...
  shader->SetScreenSize(fcyVec2((float)m_pRenderDev->GetBufferWidth(),
...
LNOINLINE bool AppFrame::PostEffect(ResTexture* rt, ResFX* shader,
                                    BlendMode blend)LNOEXCEPT {
  if (!m_bRenderStarted) {
...
LNOINLINE bool AppFrame::PostEffectCapture()LNOEXCEPT {
  if (!m_bRenderStarted || m_bPostEffectCaptureStarted) {
    LERROR(
      "PostEffectCapture: 非法调用 (RenderStarted=%d,PostEffectCaptureStarted=%d)",
      m_bRenderStarted,
      m_bPostEffectCaptureStarted);
    return false;
  }
...
LNOINLINE bool AppFrame::PostEffectApply(ResFX* shader,
                                         BlendMode blend)LNOEXCEPT {
  if (!m_bRenderStarted || !m_bPostEffectCaptureStarted) {
    LERROR(
      "PostEffectApply: 非法调用 (RenderStarted=%d,PostEffectCaptureStarted=%d)",
      m_bRenderStarted,
      m_bPostEffectCaptureStarted);
    return false;
  }

  if (m_stRenderTargetStack.empty() || m_stRenderTargetStack.back() !=
      m_PostEffectBuffer) {
...
bool AppFrame::Init()LNOEXCEPT {
  ...
  if (!m_ResourceMgr.LoadFile(LLAUNCH_SCRIPT, tMemStream))
    return false;
  if (!SafeCallScript((fcStr)tMemStream->GetInternalBuffer(),
                      (size_t)tMemStream->GetLength(),
                      "launch"))
    return false;
  ...
  LINFO("CPU %m %m / GPU %m", stCPUInfo.CPUBrandString, stCPUInfo.CPUString,
    m_pRenderDev->
    GetDeviceName());
  ...
  // 创建手柄输入
  fuInt iJoyStickCount = ::min(
    2U, m_pInputSys->GetDeviceCount(F2DINPUTDEVTYPE_GAMECTRL));
  for (fuInt i = 0; i < iJoyStickCount; ++i) {
    LINFO("侦测到手柄 设备名：%s 产品名：%s", m_pInputSys->GetDeviceName(
        F2DINPUTDEVTYPE_GAMECTRL, i),
  ...
  if (m_bSplashWindowEnabled) // 显示过载入窗口
  {
    ...
    // 改变显示模式到全屏
    if (!m_OptionWindowed)
      ChangeVideoMode((int)m_OptionResolution.x, (int)m_OptionResolution.y,
                      m_OptionWindowed,
                      m_OptionVsync);
  }
...
bool AppFrame::SafeCallScript(const char* source, size_t len,
                              const char* desc)LNOEXCEPT {
...
#pragma region 游戏循环
fBool AppFrame::OnUpdate(fDouble ElapsedTime, f2dFPSController* pFPSController,
                         f2dMsgPump* pMsgPump) {
#if (defined LDEVVERSION) || (defined LDEBUG)
  ...
  while (FCYOK(pMsgPump->GetMsg(&tMsg))) {
    switch (tMsg.Type) {
      ...
      case F2DMSG_WINDOW_ONKEYDOWN:
        // ctrl+enter全屏
        if (tMsg.Param1 == VK_RETURN && !m_KeyStateMap[VK_RETURN] &&
            m_KeyStateMap[VK_CONTROL])
          // 防止反复触发
          ChangeVideoMode((int)m_OptionResolution.x, (int)m_OptionResolution.y,
                          !m_OptionWindowed,
                          m_OptionVsync);
        ...
        if (0 < tMsg.Param1 && tMsg.Param1 < _countof(m_KeyStateMap) &&
            !(LJOYSTICK1_MAPPING_START <= tMsg.Param1 && tMsg.Param1 <=
              LJOYSTICK1_MAPPING_END) &&
            // joystick1映射区域
            !(LJOYSTICK2_MAPPING_START <= tMsg.Param1 && tMsg.Param1 <=
              LJOYSTICK2_MAPPING_END))
          // joystick2映射区域
        {
      ...
      case F2DMSG_WINDOW_ONMOUSEMOVE:
        ...
        m_MousePosition.y = m_OptionResolution.y - (float)static_cast<fInt>(tMsg
                              .Param2);
        // ! 潜在大小不匹配问题
        break;
      ...
      case F2DMSG_JOYSTICK_ONBUTTONUP:
        do {
          f2dInputJoystick* pJoystick = (f2dInputJoystick*)tMsg.Param2;
          if (tMsg.Param1 < LJOYSTICK1_MAPPING_END - LJOYSTICK1_MAPPING_START +
              1 - 4) {
            if (pJoystick == m_Joystick[0]) {
...
fBool AppFrame::
OnRender(fDouble ElapsedTime, f2dFPSController* pFPSController) {
#if (defined LDEVVERSION) || (defined LDEBUG)
  ...
  if (m_bShowCollider) {
    ...
    m_Graph2D->Begin();
    m_GameObjectPool->DrawGroupCollider(m_Graph2D, m_GRenderer, 1,
                                        fcyColor(150, 163, 73, 164));
    // GROUP_ENEMY_BULLET
    m_GameObjectPool->DrawGroupCollider(m_Graph2D, m_GRenderer, 2,
                                        fcyColor(150, 163, 73, 164));
    // GROUP_ENEMY
    m_GameObjectPool->DrawGroupCollider(m_Graph2D, m_GRenderer, 5,
                                        fcyColor(150, 163, 73, 20));
    // GROUP_INDES
    m_GameObjectPool->DrawGroupCollider(m_Graph2D, m_GRenderer, 4,
                                        fcyColor(100, 175, 15, 20));
    // GROUP_PLAYER
    m_Graph2D->End();
