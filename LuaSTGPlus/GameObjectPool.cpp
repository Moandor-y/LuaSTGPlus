...
static inline bool RenderListSortFunc(GameObject* p1, GameObject* p2)LNOEXCEPT {
  ...
  return (p1->layer < p2->layer) || (
           (p1->layer == p2->layer) && (p1->uid < p2->uid));
}
...
static inline bool CollisionCheck(GameObject* p1, GameObject* p2)LNOEXCEPT {
  ...
  if (p1->rect) {
    if (p2->rect)
      return OBBHitTest(pos1, size1, (float)p1->rot, pos2, size2,
                        (float)p2->rot);
    else
...
GameObjectBentLaser* GameObjectBentLaser::AllocInstance() {
  // ! 潜在bad_alloc
  GameObjectBentLaser* pRet = new(s_GameObjectBentLaserPool.Alloc())
    GameObjectBentLaser();
  return pRet;
}
...
bool GameObjectBentLaser::Render(const char* tex_name, BlendMode blend,
                                 fcyColor c, float tex_left, float tex_top,
                                 float tex_width, float tex_height,
                                 float scale)LNOEXCEPT {
  // 忽略只有一个节点的情况
  if (m_Queue.Size() <= 1)
    return true;
  ...
  for (size_t i = 0; i < m_Queue.Size() - 1; ++i) {
    ...
    } else // 否则，参考第三个点
    {
      ...
      // 修正交叉的情况
      float cross1 = fcyVec2(renderVertex[1].x - renderVertex[0].x,
                             renderVertex[1].y - renderVertex[0].y) *
                     fcyVec2(renderVertex[2].x - renderVertex[3].x,
                             renderVertex[2].y - renderVertex[3].y);
      float cross2 = fcyVec2(renderVertex[2].x - renderVertex[0].x,
                             renderVertex[2].y - renderVertex[0].y) *
                     fcyVec2(renderVertex[1].x - renderVertex[3].x,
                             renderVertex[1].y - renderVertex[3].y);
      if (cross2 > cross1) {
...
bool GameObjectBentLaser::CollisionCheck(float x, float y, float rot, float a,
                                         float b, bool rect)LNOEXCEPT {
  // 忽略只有一个节点的情况
  if (m_Queue.Size() <= 1)
    return false;
...
bool GameObjectBentLaser::BoundCheck()LNOEXCEPT {
  ...
  for (size_t i = 0; i < m_Queue.Size(); ++i) {
    LaserNode& n = m_Queue[i];
    if (n.pos.x >= tBound.a.x && n.pos.x <= tBound.b.x && n.pos.y <= tBound.a.y
        && n.pos.y >= tBound.b.y)
      return true;
  }
...
void GameObjectPool::BoundCheck()LNOEXCEPT {
  ...
  GameObject* p = m_pObjectListHeader.pObjectNext;
  while (p && p != &m_pObjectListTail) {
    if ((p->x < m_BoundLeft || p->x > m_BoundRight || p->y < m_BoundBottom || p
         ->y > m_BoundTop) && p->bound) {
      // 越界设置为DEL状态
      p->status = STATUS_DEL;
  ...
  while (pA && pA != pATail) {
    ...
    while (pB && pB != pBTail) {
      if (::CollisionCheck(pA, pB)) {
        ...
        lua_rawgeti(L, -5, pB->id + 1);
        // ot t(object) t(class) f(colli) t(object) t(object)
...
int GameObjectPool::New(lua_State* L)LNOEXCEPT {
  ...
  if (!lua_istable(L, 1))
    return luaL_error(
      L, "invalid argument #1, luastg object class required for 'New'.");
  ...
  LIST_INSERT_BEFORE(&m_pCollisionListTail[p->group], p, Collision);
  // 为保证兼容性，对Collision也做排序
...
int GameObjectPool::Del(lua_State* L)LNOEXCEPT {
  if (!lua_istable(L, 1))
    return luaL_error(
      L, "invalid argument #1, luastg object required for 'Del'.");
...
int GameObjectPool::Kill(lua_State* L)LNOEXCEPT {
  if (!lua_istable(L, 1))
    return luaL_error(
      L, "invalid argument #1, luastg object required for 'Kill'.");
  lua_rawgeti(L, 1, 2); // t(object) ... id
  GameObject* p = m_ObjectPool.Data((size_t)luaL_checknumber(L, -1));
  lua_pop(L, 1); // t(object) ...
  if (!p)
...
int GameObjectPool::IsValid(lua_State* L)LNOEXCEPT {
  if (lua_gettop(L) != 1)
    return luaL_error(
      L, "invalid argument count, 1 argument required for 'IsValid'.");
  if (!lua_istable(L, -1)) {
    lua_pushboolean(L, 0);
    return 1;
  }
...
bool GameObjectPool::SetV(size_t id, double v, double a,
                          bool updateRot)LNOEXCEPT {
  GameObject* p = m_ObjectPool.Data(id);
  if (!p)
    return false;
  a *= LDEGREE2RAD;
...
bool GameObjectPool::BoxCheck(size_t id, double left, double right, double top,
                              double bottom, bool& ret)LNOEXCEPT {
  GameObject* p = m_ObjectPool.Data(id);
  if (!p)
    return false;
...
int GameObjectPool::SetAttr(lua_State* L)LNOEXCEPT {
  ...
  if (!p)
    return luaL_error(
      L, "invalid lstg object for '__newindex' meta operation.");
      ...
      do {
        ...
        if (!p->res || strcmp(name, p->res->GetResName().c_str()) != 0) {
          ...
          if (!p->ChangeResource(name))
            return luaL_error(
              L, "can't find resource '%s' in image/animation/particle pool.",
...
void GameObjectPool::DrawGroupCollider(f2dGraphics2D* graph,
                                       f2dGeometryRenderer* grender,
                                       int groupId, fcyColor fillColor) {
  ...
  while (p && p != pTail) {
    if (p->colli) {
      if (p->rect) {
        ...
        grender->FillCircle(graph, fcyVec2((float)p->x, (float)p->y),
                            (float)p->col_r, fillColor, fillColor,
