...
public:
  ResSprite(const char* name, fcyRefPointer<f2dSprite> sprite, double hx,
            double hy, bool rect)
    : Resource(ResourceType::Sprite, name), m_Sprite(sprite), m_HalfSizeX(hx),
      m_HalfSizeY(hy), m_bRectangle(rect) {
...
public:
  ResAnimation(const char* name, fcyRefPointer<ResTexture> tex, float x,
...
public:
  ...
  const ParticleInfo& GetParticleInfo() const LNOEXCEPT {
...
public:
  ResParticle(const char* name, const ParticleInfo& pinfo,
              fcyRefPointer<f2dSprite> sprite, BlendMode bld, double a,
              double b, bool rect = false);
};
...
public:
  ...
  public:
    static void ReadDefine(const std::wstring& data,
                           std::unordered_map<wchar_t, f2dGlyphInfo>& out,
                           std::wstring& tex);
  ...
  public:
    ...
    fResult QueryGlyph(f2dGraphics* pGraph, fCharW Character,
                       f2dGlyphInfo* InfoOut);

  public:
    HGEFont(std::unordered_map<wchar_t, f2dGlyphInfo>&& org,
...
public:
  ...
  public:
    ...
    fResult SetPosition(F2DSEEKORIGIN Origin, fInt Offset) {
      return m_pDecoder->SetPosition(Origin, Offset);
    }

    // 对Read作处理
    fResult Read(fData pBuffer, fuInt SizeToRead, fuInt* pSizeRead);
  public:
    BGMWrapper(fcyRefPointer<f2dSoundDecoder> pOrg, fDouble LoopStart,
               fDouble LoopEnd);
  };
...
private:
  ...
  // 特殊对象绑定
  std::vector<f2dEffectParamValue*> m_pBindingPostEffectTexture;
  // POSTEFFECTTEXTURE
...
public:
  ...
  /// @brief 检查资源是否存在
  /// @warning 注意t可以是非法枚举量
  bool CheckResourceExists(ResourceType t,
                           const std::string& name) const LNOEXCEPT {
    switch (t) {
  ...
  /// @param path 路径
  /// @param mipmaps 纹理链
  bool LoadTexture(const char* name, const std::wstring& path,
                   bool mipmaps = true)LNOEXCEPT;

  /// @brief 装载纹理（UTF-8）
  LNOINLINE bool LoadTexture(const char* name, const char* path,
                             bool mipmaps = true)LNOEXCEPT;

  /// @brief 装载图像
  LNOINLINE bool LoadImage(const char* name, const char* texname,
                           double x, double y, double w, double h, double a,
                           double b, bool rect = false)LNOEXCEPT;
  ...
  LNOINLINE bool LoadAnimation(const char* name, const char* texname,
                               ...
                               bool rect = false)LNOEXCEPT;

  /// @brief 装载背景音乐
  bool LoadMusic(const char* name, const std::wstring& path, double start,
                 double end)LNOEXCEPT;

  LNOINLINE bool LoadMusic(const char* name, const char* path, double start,
                           double end)LNOEXCEPT;
  ...
  /// @brief 装载粒子
  bool LoadParticle(const char* name, const std::wstring& path,
                    const char* img_name, double a, double b,
                    bool rect = false)LNOEXCEPT;

  LNOINLINE bool LoadParticle(const char* name, const char* path,
                              const char* img_name, double a, double b,
                              bool rect = false)LNOEXCEPT;

  /// @brief 装载纹理字体(HGE)
  bool LoadSpriteFont(const char* name, const std::wstring& path,
                      bool mipmaps = true)LNOEXCEPT;
  ...
  LNOINLINE bool LoadSpriteFont(const char* name, const char* path,
                                bool mipmaps = true)LNOEXCEPT;
  ...
  /// @brief 装载TTF字体
  bool LoadTTFFont(const char* name, const std::wstring& path, float width,
                   float height)LNOEXCEPT;

  LNOINLINE bool LoadTTFFont(const char* name, const char* path, float width,
                             float height)LNOEXCEPT;
...
public:
  ...
  const std::wstring& GetPathLowerCase() const LNOEXCEPT {
    return m_PathLowerCase;
  }
...
public:
  float GetGlobalImageScaleFactor() const LNOEXCEPT {
    return m_GlobalImageScaleFactor;
  }

  void SetGlobalImageScaleFactor(float s)LNOEXCEPT {
    m_GlobalImageScaleFactor = s;
  }

  float GetGlobalSoundEffectVolume() const LNOEXCEPT {
    return m_GlobalSoundEffectVolume;
  }

  void SetGlobalSoundEffectVolume(float s)LNOEXCEPT {
    m_GlobalSoundEffectVolume = s;
  }
  ...
  LNOINLINE bool LoadFile(const wchar_t* path,
                          fcyRefPointer<fcyMemStream>& outBuf)LNOEXCEPT;

  /// @brief 装载文件（UTF8）
  /// @param[in] path 路径
  /// @param[out] outBuf 输出缓冲
  LNOINLINE bool LoadFile(const char* path,
                          fcyRefPointer<fcyMemStream>& outBuf)LNOEXCEPT;
